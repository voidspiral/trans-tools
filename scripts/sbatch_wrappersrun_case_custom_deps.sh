#!/bin/bash
# Local single-node sbatch case (custom deps parameters).
# Test assumption: management node and login node are the same local machine.
# Real HPC deployments run scheduler/login/compute on separate nodes; this case focuses on
# custom deps/fakefs options, not distributed throughput or multi-node contention.
#SBATCH --job-name=wrappersrun-case-custom
#SBATCH --nodes=1
#SBATCH --ntasks=2
#SBATCH --time=00:00:15
#SBATCH --output=logs/wrappersrun-case-custom-%j.out
#SBATCH --error=logs/wrappersrun-case-custom-%j.err
...
echo "[CASE custom] [$(date '+%F %T')] STAGE=deps-precheck"
...
# Buffer above ~3.5M fails against this agent; width 32 also fails (protocol/agent limits).
export WRAPPERSRUN_DEPS_BUFFER=3M
export WRAPPERSRUN_DEPS_WIDTH=48
export WRAPPERSRUN_DEPS_PORT=2007
export WRAPPERSRUN_DEPS_INSECURE=true
export WRAPPERSRUN_FAKEFS_DIRECT_MODE=1

echo "[CASE custom] [$(date '+%F %T')] STAGE=mount-check"
df -h
findmnt -t fuse.fakefs || true

echo "[CASE custom] [$(date '+%F %T')] STAGE=wrappersrun"
"${PROJECT_DIR}/scripts/wrappersrun.sh" -n 2 "${MPI_BIN}"
echo "[CASE custom] [$(date '+%F %T')] STAGE=pre-epilog-mount-check"
df -h
findmnt -t fuse.fakefs || true
echo "[CASE custom] [$(date '+%F %T')] STAGE=mpi-finished"
